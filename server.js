...
function parseSpintax(text) {
    ...
    while (result.includes('{') && limit-- > 0)
        result = result.replace(/\{([^{}]+)\}/g, (_, c) => { const o = c.split('|'); return o[Math.floor(Math.random()*o.length)]; });
    return result;
}
...
function seedData() {
    ...
    if (adminIdx === -1) {
        ...
        users[adminIdx].password=freshHash; users[adminIdx].role='admin';
        users[adminIdx].licenseActive=true; users[adminIdx].quotaHarian=999999;
...
function cekResetQuota(user){ const hari=new Date().toDateString(); if(user.lastReset!==hari){user.quotaTerpakai=0;user.lastReset=hari;} return user; }
...
function getRotasiSock(userId){
    ...
    const idx=ses.rotasiIdx%aktif.length; ses.rotasiIdx++;
...
function notifyPoolUsers(poolId,status,nomor){
    const users=readJSON('data/users.json'); const ps=getPoolSession(poolId);
    users.filter(u=>u.assignedWA===poolId).forEach(u=>{ const ses=getUserSession(u.id); ses.sock=ps.sock; ses.status=status; ses.poolId=poolId; io.to(`user:${u.id}`).emit('status',{status,msg:status==='connected'?`✅ Terhubung: ${nomor}`:'Menghubungkan...'}); });
}
...
async function connectPoolQR(poolId){
    ...
    ps.sock.ev.on('connection.update',async(update)=>{
        ...
        if(qr){ ps.status='qr'; const qrImage=await qrcode.toDataURL(qr); io.to('admin').emit('pool-qr',{poolId,qr:qrImage}); const pool=readJSON('data/wa-pool.json'); const idx=pool.findIndex(p=>p.id===poolId); if(idx!==-1){pool[idx].status='qr';writeJSON('data/wa-pool.json',pool);} io.to('admin').emit('pool-status',{poolId,status:'qr'}); }
        if(connection==='close'){ ps.status='disconnected'; notifyPoolUsers(poolId,'disconnected',''); const kode=lastDisconnect?.error?.output?.statusCode; if(kode!==DisconnectReason.loggedOut){setTimeout(()=>connectPoolQR(poolId),5000);}else{const pool=readJSON('data/wa-pool.json');const idx=pool.findIndex(p=>p.id===poolId);if(idx!==-1){pool[idx].status='expired';writeJSON('data/wa-pool.json',pool);}io.to('admin').emit('pool-status',{poolId,status:'expired'});} }
        if(connection==='open'){ ps.status='connected'; const nomorWA=ps.sock.user?.id?.split(':')[0]||''; const pool=readJSON('data/wa-pool.json'); const idx=pool.findIndex(p=>p.id===poolId); if(idx!==-1){pool[idx].status='connected';pool[idx].nomor=nomorWA;writeJSON('data/wa-pool.json',pool);} io.to('admin').emit('pool-qr',{poolId,qr:null}); io.to('admin').emit('pool-status',{poolId,status:'connected',nomor:nomorWA}); notifyPoolUsers(poolId,'connected',nomorWA); }
    });
    ps.sock.ev.on('messages.upsert',({messages,type})=>{
        if(type!=='notify') return;
        for(const msg of messages){ if(msg.key.fromMe) continue;
            ...
            const users=readJSON('data/users.json'); const seenIds=new Set();
...
async function connectPersonal(userId){
    ...
    ses.sock.ev.on('messages.upsert',({messages,type})=>{
        ...
        for(const msg of messages){ if(msg.key.fromMe) continue;
            ...
            const text=msg.message?.conversation||msg.message?.extendedTextMessage?.text||msg.message?.imageMessage?.caption||'[Media]';
            const item={from,text,time:new Date().toLocaleTimeString('id-ID')};
            ses.inbox.unshift(item); if(ses.inbox.length>100)ses.inbox.pop(); io.to(`user:${userId}`).emit('inbox',item);
        }
    });
}
...
app.post('/api/login',async(req,res)=>{
    ...
    if(user.banned) return res.json({success:false,error:'Akun kamu dinonaktifkan. Hubungi admin.'});
...
app.get('/api/me',requireLogin,(req,res)=>{
    ...
    if(user.banned){ req.session.destroy(); return res.json({success:false,error:'banned'}); }
...
app.post('/api/aktivasi',requireLogin,(req,res)=>{
    ...
    res.json({success:true,msg:`✅ Lisensi ${lic.plan} aktif! Berlaku ${lic.durasiHari} hari.`});
});
...
app.post('/api/admin/wa-pool/add',requireAdmin,async(req,res)=>{
    const pool=readJSON('data/wa-pool.json'); const poolId=uuidv4();
    pool.push({id:poolId,nomor:'',status:'pending',assignedTo:null,createdAt:new Date().toISOString()});
    writeJSON('data/wa-pool.json',pool); res.json({success:true,poolId,msg:'Slot ditambahkan. Scan QR di bawah.'}); connectPoolQR(poolId);
});
app.delete('/api/admin/wa-pool/:id',requireAdmin,(req,res)=>{
    const {id}=req.params; let pool=readJSON('data/wa-pool.json');
    const ps=getPoolSession(id); if(ps.sock){try{ps.sock.end();}catch(e){}delete poolSessions[id];}
    pool=pool.filter(p=>p.id!==id); writeJSON('data/wa-pool.json',pool);
    const users=readJSON('data/users.json');
    ...
    writeJSON('data/users.json',users); res.json({success:true});
});
app.post('/api/admin/wa-pool/assign',requireAdmin,(req,res)=>{
    const {poolId,userId}=req.body; if(!poolId||!userId) return res.json({success:false,error:'poolId dan userId wajib!'});
    const pool=readJSON('data/wa-pool.json'); const p=pool.find(p=>p.id===poolId);
    if(!p) return res.json({success:false,error:'Pool tidak ditemukan!'}); if(p.status!=='connected') return res.json({success:false,error:'Nomor belum connected!'});
    const users=readJSON('data/users.json'); const idx=users.findIndex(u=>u.id===userId);
    ...
    users[idx].assignedWA=poolId; writeJSON('data/users.json',users);
    pool[pool.findIndex(p=>p.id===poolId)].assignedTo=userId; writeJSON('data/wa-pool.json',pool);
    const ps2=getPoolSession(poolId); const ses=getUserSession(userId);
    ses.sock=ps2.sock; ses.status=ps2.status; ses.poolId=poolId;
    io.to(`user:${userId}`).emit('status',{status:ps2.status,msg:ps2.status==='connected'?`✅ Terhubung: ${p.nomor}`:'Menghubungkan...'});
...
app.post('/api/admin/wa-pool/unassign',requireAdmin,(req,res)=>{
    const {userId}=req.body; const users=readJSON('data/users.json'); const idx=users.findIndex(u=>u.id===userId);
    if(idx===-1) return res.json({success:false,error:'User tidak ditemukan!'});
    users[idx].assignedWA=null; writeJSON('data/users.json',users);
    const ses=getUserSession(userId); ses.sock=null; ses.status='disconnected'; ses.poolId=null;
...
app.get('/api/admin/users/:id',requireAdmin,(req,res)=>{
    const users=readJSON('data/users.json'); const pool=readJSON('data/wa-pool.json');
    ...
    res.json({success:true,user:{id:user.id,username:user.username,role:user.role,plan:user.plan||'-',licenseActive:user.licenseActive,licenseExpiry:user.licenseExpiry,quotaHarian:user.quotaHarian,quotaTerpakai:user.quotaTerpakai,assignedNomor:wp?.nomor||null,banned:user.banned||false,createdAt:user.createdAt}});
});
...
app.post('/api/admin/users/:id/reset-password',requireAdmin,async(req,res)=>{
    ...
    const users=readJSON('data/users.json'); const idx=users.findIndex(u=>u.id===req.params.id);
    if(idx===-1) return res.json({success:false,error:'User tidak ditemukan!'});
    if(users[idx].role==='admin') return res.json({success:false,error:'Tidak bisa reset password admin!'});
    users[idx].password=await bcrypt.hash(newPassword,10); writeJSON('data/users.json',users);
...
app.delete('/api/admin/users/:id',requireAdmin,(req,res)=>{
    const users=readJSON('data/users.json'); const user=users.find(u=>u.id===req.params.id);
    if(!user) return res.json({success:false,error:'User tidak ditemukan!'});
    if(user.role==='admin') return res.json({success:false,error:'Tidak bisa hapus akun admin!'});
    const ses=getUserSession(user.id); if(ses.sock){try{ses.sock.end();}catch(e){}}
...
app.get('/api/status',requireLogin,(req,res)=>{
    const users=readJSON('data/users.json'); const user=users.find(u=>u.id===req.session.userId);
    const pool=readJSON('data/wa-pool.json'); const poolAktif=pool.filter(p=>p.status==='connected');
    if(user?.assignedWA){ const ps=getPoolSession(user.assignedWA); const p=pool.find(p=>p.id===user.assignedWA); return res.json({status:ps.status,nomor:p?.nomor||'-',mode:'pool',poolAktif:poolAktif.length}); }
...
app.post('/api/blast',requireLogin,async(req,res)=>{
    ...
    if(user.assignedWA){ const ps=getPoolSession(user.assignedWA); if(ps.status!=='connected') return res.json({success:false,error:'Nomor WA pool belum connected!'}); sockGetter=()=>ps.sock; }
        ...
        sockGetter=()=>{ const sock=getRotasiSock(req.session.userId); if(sock) return sock; const ses=getUserSession(req.session.userId); return ses.status==='connected'?ses.sock:null; };
        if(!sockGetter()) return res.json({success:false,error:'WA belum terhubung! Scan QR atau tambah nomor ke pool.'});
...
app.post('/api/reset-sesi',requireLogin,async(req,res)=>{
    const userId=req.session.userId; const users=readJSON('data/users.json'); const user=users.find(u=>u.id===userId);
...
async function jalankanBlast(userId,sockGetter,pesan,numbers){
    ...
    for(let i=0;i<numbers.length;i++){
        ...
        let status='gagal';
        try{ await sock.sendMessage(`${numbers[i]}@s.whatsapp.net`,{text:pesanFinal}); status='sukses'; const users=readJSON('data/users.json'); const idx=users.findIndex(u=>u.id===userId); if(idx!==-1){users[idx].quotaTerpakai=(users[idx].quotaTerpakai||0)+1;writeJSON('data/users.json',users);} }catch(err){}
...
io.on('connection',(socket)=>{
    ...
    const users=readJSON('data/users.json'); const user=users.find(u=>u.id===userId);
    if(user?.role==='admin') socket.join('admin');
    if(user?.assignedWA){
        const ps=getPoolSession(user.assignedWA); const pool=readJSON('data/wa-pool.json'); const p=pool.find(p=>p.id===user.assignedWA);
        ...
        const pool=readJSON('data/wa-pool.json'); const aktif=pool.filter(p=>p.status==='connected');

...
app.post('/api/register',async(req,res)=>{
    ...
    writeJSON('data/users.json',users); res.json({success:true,msg:'Akun berhasil dibuat!'});
});
...
app.post('/api/aktivasi',requireLogin,(req,res)=>{
    ...
    res.json({success:true,msg:`\u2705 Lisensi ${lic.plan} aktif! ${lic.durasiHari} hari. ${lic.quotaHarian}/hari.`});
});
...
app.post('/api/member/order',async(req,res)=>{
    ...
    orders.push({id:orderId,username,wa,metode,plan,harga:parseInt(harga),quota:parseInt(quota),hari:parseInt(hari),passwordHash:await bcrypt.hash(password,10),status:'pending',createdAt:new Date().toISOString()});
    writeJSON('data/orders.json',orders);
    const tgToken=process.env.TELEGRAM_BOT_TOKEN, tgChat=process.env.TELEGRAM_CHAT_ID;
    if(tgToken&&tgChat){ fetch(`https://api.telegram.org/bot${tgToken}/sendMessage`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({chat_id:tgChat,text:`\ud83d\udce6 Order Baru!\n\ud83d\udc64 ${username}\n\ud83d\udcb0 ${plan}\n\ud83d\udcf1 ${wa}\n\ud83d\udcb3 ${metode}\n\ud83d\udd11 ${orderId.slice(-8)}`})}).catch(()=>{}); }
    res.json({success:true,orderId,msg:'Order diterima!'});
});
...
app.post('/api/admin/orders/:id/confirm',requireAdmin,async(req,res)=>{
    ...
    users.push({id:uuidv4(),username:order.username,password:order.passwordHash,role:'user',licenseKey:`ORDER-${order.id.slice(-8)}`,licenseActive:true,licenseExpiry:expiry.toISOString(),plan:order.plan,quotaHarian:order.quota,quotaTerpakai:0,lastReset:'',assignedWA:null,banned:false,createdAt:new Date().toISOString()});
    writeJSON('data/users.json',users);
    orders[idx].status='done'; orders[idx].confirmedAt=new Date().toISOString();
    writeJSON('data/orders.json',orders);
    const tgToken=process.env.TELEGRAM_BOT_TOKEN, tgChat=process.env.TELEGRAM_CHAT_ID;
    if(tgToken&&tgChat){ fetch(`https://api.telegram.org/bot${tgToken}/sendMessage`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({chat_id:tgChat,text:`\u2705 Dikonfirmasi!\n\ud83d\udc64 ${order.username}\n\ud83d\udce6 ${order.plan}`})}).catch(()=>{}); }
    res.json({success:true,msg:`\u2705 Akun ${order.username} diaktifkan!`});
});
...
app.post('/api/blast',requireLogin,async(req,res)=>{
    ...
    if(sisa<=0) return res.json({success:false,error:'Quota habis!'});
    const blastNomor=allNomor.slice(0,sisa); ses.blastLog=[];ses.isBlasting=true;
    res.json({success:true,total:blastNomor.length,catatan:blastNomor.length<allNomor.length?`Hanya ${blastNomor.length} dikirim (sisa quota)`:null});
